  ...
  if (view === 'detail') return (
    <div style={{ minHeight:'100vh', background:'#05050a', color:'#f0eeff', fontFamily:'sans-serif' }}>
      ...
      <div style={{ maxWidth:'800px', margin:'0 auto', padding:'32px 24px' }}>
        ...
        <div style={{ background:'rgba(13,13,24,0.8)', border:'1px solid rgba(191,90,242,0.15)', borderRadius:'16px', padding:'28px', marginBottom:'20px' }}>
        ...
        {/* Tabs */}
        <div style={{ display:'flex', gap:'4px', background:'rgba(13,13,24,0.8)', border:'1px solid rgba(191,90,242,0.15)', borderRadius:'12px', padding:'4px', marginBottom:'20px' }}>
        ...
        {noteView === 'notes' && (
          <div style={{ background:'rgba(13,13,24,0.8)', border:'1px solid rgba(191,90,242,0.15)', borderRadius:'16px', padding:'28px' }}>
            {note.content.split('\n').map((line, i) => {
        ...
        {noteView === 'flashcards' && (
          <div>
            ...
            <div onClick={()=>setFlipped(!flipped)} style={{ background:flipped?'rgba(191,90,242,0.1)':'rgba(13,13,24,0.8)', border:`1px solid ${flipped?'rgba(191,90,242,0.4)':'rgba(191,90,242,0.15)'}`, borderRadius:'20px', padding:'48px 32px', textAlign:'center', cursor:'pointer', minHeight:'200px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', marginBottom:'20px', transition:'all .3s' }}>
    ...
    <div style={{ minHeight:'100vh', background:'#05050a', color:'#f0eeff', fontFamily:'sans-serif' }}>
      ...
      <div style={{ maxWidth:'900px', margin:'0 auto', padding:'32px 24px' }}>
        ...
        <input type="text" placeholder="🔍  Search notes..." value={search} onChange={e=>setSearch(e.target.value)}
          style={{ width:'100%', padding:'13px 16px', background:'rgba(13,13,24,0.8)', border:'1px solid rgba(191,90,242,0.15)', borderRadius:'12px', color:'#f0eeff', fontSize:'14px', outline:'none', boxSizing:'border-box', marginBottom:'20px' }}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
          {filtered.map((n,i) => (
            <div key={i} onClick={()=>openNote(n)}
              style={{ background:'rgba(13,13,24,0.8)', border:'1px solid rgba(191,90,242,0.15)', borderRadius:'16px', padding:'20px 24px', display:'flex', alignItems:'center', gap:'20px', cursor:'pointer' }}
